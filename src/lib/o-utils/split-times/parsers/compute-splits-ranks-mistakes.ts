...
export default function computeSplitsRanksMistakes(runners: Runner[]): Runner[] {
	const rankedRunners = computeRunnersRanks(runners);

	const [splitRankedRunners, supermanSplits] = computeSplitRanksAndTimeBehind(rankedRunners);

	const overallSplitRankedRunners = computeOverallSplitRanks(splitRankedRunners, supermanSplits);

	const runnersWithMistakes = computeRunnersMistakes(overallSplitRankedRunners, supermanSplits);
